...
use graphlite_core::record::{
    EdgeRecord, NodeRecord, FLAG_ACTIVE, FLAG_DIRECTED, NO_VECTOR_OFFSET,
};
...
fn test_error_matching() {
    fn simulate_lookup(id: NodeId) -> Result<(), GraphLiteError> {
        if id.as_u32() > 100 {
            Err(GraphLiteError::NodeNotFound(id))
        } else {
            Ok(())
        }
    }
    ...
    match err {
        GraphLiteError::NodeNotFound(id) => assert_eq!(id, NodeId::new(500)),
