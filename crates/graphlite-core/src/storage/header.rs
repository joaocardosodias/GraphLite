...
/// Magic bytes identifying a valid GraphLite binary file (`.graph`).
...
impl GraphHeader {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        if magic != GRAPH_MAGIC {
            return Err(GraphLiteError::InvalidMagicBytes {
        ...
        if version != GRAPH_VERSION {
            return Err(GraphLiteError::UnsupportedVersion {
...
mod tests {
    ...
    fn test_unsupported_version_detection() {
        ...
        let err = GraphHeader::from_bytes(&bytes).unwrap_err();
        match err {
            GraphLiteError::UnsupportedVersion { expected, found } => {
