...
pub fn execute_serve(db_path: &Path, args: &ServeArgs) -> Result<()> {
    ...
    let engine = Arc::new(RwLock::new(GraphLiteEngine::open_or_create(
        db_path, config,
    )?));
    ...
    println!("========================================================");
    println!("  🚀 GraphLite Embedded REST API Server Running");
    println!("========================================================");
                        ...
                        match run_ingest_pass(&db_path_buf, &ingest_args, &embedder_clone, false) {
                            Ok(modified) => {
                                if modified {
                                    ...
                                    if let Ok(new_eng) =
                                        GraphLiteEngine::open_or_create(&db_path_buf, new_cfg)
                                    {
