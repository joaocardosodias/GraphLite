...
/// Centralized error enum for all Graphite operations.
#[derive(Debug, Error)]
...
/// Convenience alias for `std::result::Result<T, GraphiteError>`.
...
mod tests {
    ...
    fn test_error_display_formatting() {
        let err = GraphiteError::VectorDimensionMismatch {
            expected: 384,
            found: 768,
        };
        assert_eq!(
        ...
        let node_err = GraphiteError::NodeNotFound(NodeId::new(42));
        assert_eq!(node_err.to_string(), "Node not found: NodeId(42)");

        let checksum_err = GraphiteError::ChecksumMismatch {
            expected: 0x12345678,
